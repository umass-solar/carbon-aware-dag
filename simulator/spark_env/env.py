...
class Environment(object):
    def __init__(self, carbon_schedule = None):
        ...
        self.carbon_usage_over_time = [] 

        #to track carbon
        self.last_CI_time = 0
        self.total_carbon_usage = 0  # we can get rid of this to speed up the env
    ...
    def create_dummy_node(self, duration, limit):
        # Create a list of tasks for the node
        dummy_tasks = []
        task_durations = []
        ...
        for i in range(limit):
            dummy_task = Task(
                idx=i,
                rough_duration=duration,
                wall_time=self.wall_time
            )
            dummy_tasks.append(dummy_task)
            task_durations.append(duration)

        # Create a single node with multiple tasks
        dummy_node = Node(
            idx=0,  # Single node index
            tasks=dummy_tasks,
            task_duration={
                'first_wave': {0: task_durations},
                'rest_wave': {0: [0]*limit},
                'fresh_durations': {0: [0]*limit}
            },
            wall_time=self.wall_time,
            np_random=self.np_random
        )

        dummy_node.parent_nodes = []
        dummy_node.child_nodes = []

        adjacency_matrix = np.array([[0]])  # No dependencies
        dummy_job_dag = JobDAG([dummy_node], adjacency_matrix, "dummy")
        dummy_node.job_dag = dummy_job_dag

        #print("Dummy node created: ", dummy_node)
        #print("Dummy job DAG created: ", dummy_node.job_dag)

        return dummy_node
    
    def time_until_next_scheduling_decision(self):
        if len(self.timeline) > 0:
            next_event_time, _ = self.timeline.peek()
            current_time = self.wall_time.curr_time
            return max(0, next_event_time - current_time)
        else:
            # if no events are scheduled, return infinity as a placeholder
            return float('inf')

    def time_until_next_carbon_reading(self):
        current_time = self.wall_time.curr_time
        
        # round the current time to the nearest (next) value in the carbon schedule keys
        keys = [key for key in self.carbon_schedule.keys() if key > current_time]
        if len(keys) > 0:
            next_time = min(keys)
            return next_time - current_time
        
        # If no further changes in carbon intensity, return infinity
        return float('inf')

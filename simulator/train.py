...
def invoke_model(actor_agent, obs, exp, printProbs=False):
    # parse observation
    ...
    if printProbs:
        # print('node_act_probs:', node_act_probs)
        # print('job_act_probs:', job_act_probs)
        # calculate node and job probability of chosen action
        selected_node_prob = np.sum(
            np.multiply(node_act_probs, node_act_vec),
            axis=1, keepdims=True
        )

        # Compute selected job action probability
        selected_job_prob = np.sum(
            np.sum(np.multiply(job_act_probs, job_act_vec), axis=2),
            axis=1, keepdims=True
        )
        print('selected_node_prob:', selected_node_prob)
        print('selected_job_prob:', selected_job_prob)
        print('max_node_prob:', np.max(node_act_probs))
        print('max_job_prob:', np.max(job_act_probs))
        print('min_node_prob:', np.min(node_act_probs))
        print('min_job_prob:', np.min(job_act_probs))
    ...
    while True:
        ...
        try:
            ...
            while not done:
                ...
                # if j%1000 == 0:
                #     node, use_exec = invoke_model(actor_agent, obs, exp, printProbs=True)
                # else:
            ...
            # report job probabilities to master
            # prob_queue.put(exp['node_act_vec'])
...
def main():
    ...
    job_prob_queues = [mp.Queue(1) for _ in range(args.num_agents)]
    ...
    for i in range(args.num_agents):
        agents.append(mp.Process(target=train_agent, args=(
            ...
            adv_queues[i], gradient_queues[i], job_prob_queues[i])))

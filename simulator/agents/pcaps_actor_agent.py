...
class PCAPSAgent(Agent):
    ...
    def get_carbon_intensity(self, current_time):
        ...
        if len(keys) > 0:
            ...
            if len(future_keys) > 48:
                ...
                L = min(future_carbon) * 0.95
                U = max(future_carbon) * 1
                # L = 280
                # U = 400
    ...
    def get_action(self, obs):
        ...
        if threshold < current_carbon:
            self.num_deferred += 1
            # do the carbon-aware action (defer job)
            return None, num_source_exec, True
        
        # parse job limit action and scale for carbon intensity
        use_exec_init = min(
            node.num_tasks - node.next_task_idx - \
        ...
        # scale use_exec by the current carbon
        if self.gamma > 0.0:
            scaled = use_exec_init * min(np.exp(self.gamma * ( self.L - current_carbon ) ), (1 - min(0.9, self.gamma)))
            diff = 0
            if not np.isnan(scaled):
                diff = use_exec_init - min(int(np.ceil(scaled)), use_exec_init)
            use_exec = int(use_exec_init - self.gamma * diff)
        else: 
            use_exec_init = use_exec

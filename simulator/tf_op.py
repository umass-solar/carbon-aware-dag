...
def expand_act_on_state(state, sub_acts):
    ...
    batch_size = tf.shape(input=state)[0]
    num_nodes = tf.shape(input=state)[1]
...
def masked_outer_product(a, b, mask):
    ...
    num_nodes = tf.shape(input=a)[1]
    num_limits = tf.shape(input=b)[1]
    ...
    # mask
    outer_product = tf.transpose(a=outer_product)
    outer_product = tf.boolean_mask(tensor=outer_product, mask=mask)
    outer_product = tf.transpose(a=outer_product)

...
private[spark] class DAGScheduler(
  ...
  // HashSet to keep track of logged stages
  private val loggedStages = new HashSet[Int]()
  ...
  private def sendToExternalApi(data: String): Unit = {
    val appId = sc.applicationId
    val url = new URL("http://192.168.1.10:14040/registerJob")
    ...
    val dataWithAppId = s"""{"appId": "$appId", "data": "$data"}"""
    outputStream.write(dataWithAppId)
    outputStream.flush()
    outputStream.close()

    val responseCode = connection.getResponseCode
    logInfo(s"Sent data to external API, response code: $responseCode")
    connection.disconnect()
  }

  private def logStageDependencies(jobId: Option[Int], stage: Stage): Unit = {
    if (!loggedStages.contains(stage.id) && jobId.isDefined) {
      val dependencies = stage.parents.map(_.id).mkString(", ")
      val stageInfo = s"Job ${jobId.get}, Stage ${stage.id} depends on stages: $dependencies"
      logInfo(stageInfo)
      sendToExternalApi(stageInfo)
      loggedStages.add(stage.id)
    }
  }
  ...
  private def submitStage(stage: Stage): Unit = {
    ...
    if (jobId.isDefined) {
      logStageDependencies(jobId, stage)

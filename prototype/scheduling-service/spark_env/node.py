...
class Node(object):
    ...
    def set_finished_tasks(self, new_finished_tasks):
        self.num_finished_tasks = min(self.num_tasks, new_finished_tasks)
        self.tasks_all_done = (self.num_finished_tasks == self.num_tasks)
        if self.tasks_all_done:
            self.node_finish_time = self.wall_time.curr_time
